...
/****************************************************************************************************************************/
int getRange(player pData, float lowerLimit, float upperLimit)
{
	float rawRange;
...
{
	currentTreas->treasVal=(getRange(pData,pData.pLvl,pData.pLvl+5))*10;
}
void weaponData(weapon *currentWeap, player pData)
{
	...
	if (max<0)
		max=0;
	currentWeap->weapVal=(getRange(pData,max,pData.pLvl))*10;
	currentWeap->weapDmg=getRange(pData,pData.pLvl*0.1,pData.pLvl*0.5);
	currentWeap->weapHP=getRange(pData,1,5);	
}
...
{	
	...
	if (max<0)
		max=0;	
	currentArmor->armorVal=getRange(pData,max,pData.pLvl+1);
	currentArmor->armorDef=getRange(pData,pData.pLvl*0.1,pData.pLvl*0.7);
	currentArmor->armorHp=getRange(pData,1,5);
}
void enemData(enemy *currentEnem, player pData)
{
	int max=pData.pLvl-5;
	...
	if (max<0)
		max=0;
	currentEnem->enemDmg=(getRange(pData,max,pData.pLvl+1))*3;
	currentEnem->enemHP=getRange(pData,max,pData.pLvl);
}
...
int checkForNextRoom(char box[][7], int x, int y)
{
	if(x==-1||x==7||y==-1||y==7)
		return 1;
	else return 0;
}
int checkForTreasure(char box[][7], int x, int y)
{
	if (box[x][y]=='T')
		return 1;
	else return 0;
...
void  generateRoom (char box[][7],int roomAlgo[][3],int start,int a,int b,player pData,weapon *currentWeap,armor *currentArmor,enemy *currentEnem,treasure *currentTreas)
{
...
int main()
{
	...
	do{
		...
		generateRoom(box,roomAlgo,start,Rx,Ry,pData,&currentWeap,&currentArmor,&currentEnem,&currentTreas);
